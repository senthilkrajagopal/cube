...
import type {
  ...
  DataSourceScaffoldedCube,
  DataSourceScaffoldOptions,
  DataSourceTable,
  DataSourceTableColumns,
  DataSourceTableRef,
  DataSourceTableType,
  DataSourceUnmappedColumn,
} from './types';
import {
  columnTypeOf,
  DatabaseSchema,
...
import type { CatalogOperation } from './catalog/queue';
import { loadsIncrementally } from './catalog/views';

/** Reads the data source's catalog through its queue. */
export type CatalogReader = <T>(operation: CatalogOperation, params?: Record<string, any>) => Promise<T>;
...
function toColumn(row: QueryColumnsResult): DataSourceColumn {
  return {
    ...
    foreignKeys: ((row.foreign_keys || []) as CatalogForeignKey[]).map(fk => ({
 ...
 * tables. Every read of the catalog goes through a queue for that data
 * source (see `CatalogQueues`), with the concurrency, timeouts and `Continue
 * wait` of Cube's own, and nothing is cached: each call answers from the data
 * source's catalog as it is now.
 *
...
export class DataSourceIntrospection implements DataSourceIntrospectionApi {
  public constructor(
    protected readonly read: CatalogReader,
  ...
  public async schemas(): Promise<string[]> {
    const names = await this.isIncremental()
      ? (await this.read<QuerySchemasResult[]>('schemas')).map(({ schema_name: schema }) => schema)
  ...
  public async tables(schemas: string[]): Promise<DataSourceTable[]> {
    ...
    let rows: CatalogTableRow[];
    if (await this.isIncremental()) {
      rows = await this.read<CatalogTableRow[]>('tables', {
        schemas: wanted.map(schema => ({ schema_name: schema })),
      });
    } else {
  ...
  public async columns(tables: DataSourceTableRef[]): Promise<DataSourceTableColumns[]> {
    ...
    if (await this.isIncremental()) {
      rows = await this.read<QueryColumnsResult[]>('columns', {
        tables: wanted.map(({ schema, table }) => ({ schema_name: schema, table_name: table })),
      });
    } else {
      const structure = await this.tablesSchema();
  ...
  protected async isIncremental(): Promise<boolean> {
    return loadsIncrementally(await this.driverFactory());
  }

  protected tablesSchema(): Promise<DatabaseStructure> {
    return this.read<DatabaseStructure>('tablesSchema');
  }
}
