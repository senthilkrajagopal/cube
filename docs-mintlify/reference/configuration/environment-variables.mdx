...
Keep this option enabled. With `false`, only *older versions* of each table are
dropped, so partitions the current data model no longer builds — rotated out of
the [build
range](/reference/data-modeling/pre-aggregations#build_range_start-and-build_range_end),
or left behind after a rollup is renamed or removed, or after its partition
granularity is changed — are never dropped. They accumulate in Cube Store and
inflate its metastore, which slows metastore operations, degrades every query
served from a pre-aggregation, and can eventually bring down the cluster.

If partitions that are still in use are being dropped, raise
[`CUBEJS_TOUCH_PRE_AGG_TIMEOUT`](/reference/configuration/environment-variables#cubejs_touch_pre_agg_timeout)
and/or add Refresh Worker capacity so every partition is touched within the
timeout, rather than disabling this option.
...
## `CUBEJS_INTROSPECTION_MAX_TABLES`

The most tables one request to the data source introspection
[`/columns`](/reference/core-data-apis/rest-api/reference#data-source-introspection)
or `/scaffold` endpoint may name.

| Possible Values           | Default in Development | Default in Production |
| ------------------------- | ---------------------- | --------------------- |
| A valid number of tables  | `100`                  | `100`                 |
...
## `CUBEJS_LOG_REDACTION`

If `true`, filter values, SQL parameters, and string literals in [SQL API][ref-sql-api]
statements are replaced with `redacted` before a log event is written, so personal data
passed in queries does not end up in log storage. Only logs are affected: events sent to
`CUBEJS_AGENT_ENDPOINT_URL`, which is how Cube Cloud receives them for Query History, keep
the original values. Member names, numeric
literals, date ranges, and the query shape are kept, as is the security context. Error
messages from a data source are logged as returned; a SQL API error that quotes the
statement has it redacted.
...
It is off by default in development mode, that is when `NODE_ENV` is not `production` or
[`CUBEJS_DEV_MODE`](#cubejs_dev_mode) is `true`, so runnable SQL statements can be read
from the console.
...
the data source over a Cube query with no `LIMIT` clause, or with one above the maximum
row limit set by
[`CUBESQL_NON_STREAMING_QUERY_MAX_ROW_LIMIT`](#cubesql_non_streaming_query_max_row_limit).

Such a query is capped at that limit, without any ordering, and a `LIMIT` above it is
clamped back down to it. Sorting, filtering, or joining that capped result outside the
data source treats an arbitrary slice of the rows as if it were the whole population, so
the answer is wrong rather than merely short. The SQL API pushes these queries down to
the data source whenever it can; this option controls what happens when it cannot. Leave
it `false` to keep returning the truncated result, or set it to `true` to fail with an
error instead.
...
[SQL API][ref-sql-api] reads data from a cube. Larger result sets are split into
several batches, which lets downstream consumers process them incrementally
instead of materializing the whole result set at once. In particular, the
[`/v1/cubesql`][ref-ref-cubesql] endpoint emits one newline-delimited JSON chunk
per batch.
